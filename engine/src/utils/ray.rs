...
impl Ray {
    ...
    pub fn cast_until<T: Fn(Point3<f32>, Vector3<i8>) -> bool>(
        ...
        if max.x.is_nan() {
            max.x = std::f32::INFINITY;
        };
        if max.y.is_nan() {
            max.y = std::f32::INFINITY;
        };
        if max.z.is_nan() {
            max.z = std::f32::INFINITY;
        };
        ...
        if delta.x.is_nan() {
            delta.x = std::f32::INFINITY;
        };
        if delta.y.is_nan() {
            delta.y = std::f32::INFINITY;
        };
        if delta.z.is_nan() {
            delta.z = std::f32::INFINITY;
        };

        let mut face = Vector3::new(0, 0, 0);

        let length = self.length as f32
            / (self.direction.x.powf(2.) + self.direction.y.powf(2.) + self.direction.z.powf(2.))
                .sqrt();
