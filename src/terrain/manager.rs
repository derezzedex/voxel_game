...
impl TerrainManager {
        ...
        if position != self.position {
            ...
            for z in -LOAD_DISTANCE..=LOAD_DISTANCE {
                for y in -LOAD_DISTANCE..=LOAD_DISTANCE {
                    for x in -LOAD_DISTANCE..=LOAD_DISTANCE {
                        let position =
                            ChunkPosition::new(position.x + x, position.y + y, position.z + z);
    ...
    pub fn mesh_chunks(&mut self, display: &renderer::Display, timer: &std::time::Instant) {
        //temp
        let mut remove_list = Vec::new();
        for c_ref in self.chunks.clone().iter() {
            let position = c_ref.key();
            if (position.x >= self.position.x - LOAD_DISTANCE
                && position.x <= self.position.x + LOAD_DISTANCE)
                && (position.y >= self.position.y - LOAD_DISTANCE
                    && position.y <= self.position.y + LOAD_DISTANCE)
                && (position.z >= self.position.z - LOAD_DISTANCE
                    && position.z <= self.position.z + LOAD_DISTANCE)
            {
                if !self.meshes.contains_key(position)
                    && !self.visible_chunks.contains_key(position)
                {
                    // self.send_to_mesh(position);
        ...
        while timer.elapsed() < std::time::Duration::from_millis(1000 / min_fps) {
            if let Ok((position, opaque, transparent)) = self.mesher.receive() {
                ...
                self.meshes
                    .insert(position, (opaque_mesh, transparent_mesh));
            } else {
                break;
            }
        }
    ...
    pub fn block_at(&self, x: f32, y: f32, z: f32) -> Option<(usize, &BlockData)> {
        ...
        if let Some(chunk_ref) = self.chunks.get(&chunk_pos) {
            let b_pos = [
                (((x % CHUNKSIZE as f32) + CHUNKSIZE as f32) % CHUNKSIZE as f32) as usize,
                (((y % CHUNKSIZE as f32) + CHUNKSIZE as f32) % CHUNKSIZE as f32) as usize,
                (((z % CHUNKSIZE as f32) + CHUNKSIZE as f32) % CHUNKSIZE as f32) as usize,
            ];
            ...
            let data = self
                ...
                .block_registry()
                .by_id(block)
                .expect(&format!("Unknown block: {}", block));
    ...
    pub fn set_block_no_dirty(&mut self, x: f32, y: f32, z: f32, block: usize) {
        let c_pos = ChunkPosition::from_world(x, y, z);
        if let Some(mut chunk) = self.chunks.get_mut(&c_pos) {
            let chunk = Arc::make_mut(chunk.value_mut());
            let b_pos = Point3::new(
                (((x % CHUNKSIZE as f32) + CHUNKSIZE as f32) % CHUNKSIZE as f32) as usize,
                (((y % CHUNKSIZE as f32) + CHUNKSIZE as f32) % CHUNKSIZE as f32) as usize,
    ...
    fn send_to_generate(&mut self, position: ChunkPosition) {
        ...
        let registry = self.registry.clone();
        self.threadpool
            .execute(move || TerrainManager::generate_chunk(position, chunks, noise, registry));
    }
        ...
        for x in 0..CHUNKSIZE {
            for y in 0..CHUNKSIZE {
                for z in 0..CHUNKSIZE {
                    ...
                    let height = range_map(elevation, [-1., 1.], [0., CHUNKSIZE as f64]).round();
        ...
        if let Some(chunk) = self.chunks.get(position) {
            ...
            self.threadpool.execute(move || {
                ...
                let air = registry
                    .block_registry()
                    .id_of("air")
                    .expect("Air missing in Registry!");
                let block_mesh = registry
                    .mesh_registry()
                    .id_of("block")
                    .expect("Block mesh missing in Registry!");
                                ...
                                match b_data.get_mesh() {
                                    b_mesh if b_mesh == block_mesh => {
                                        for direction in &[
                                            ...
                                            let facing =
                                                Vector3::new(x as isize, y as isize, z as isize)
                                                    + direction.normal();
                                            let facing = chunk.check_block(
                                                facing.x,
                                                facing.y,
                                                facing.z,
                                                neighbors.clone(),
                                            ); //TODO: Fix this clone
                                            let facing_data = registry
                                                .block_registry()
                                                .by_id(facing)
                                                .expect("Unknown block in chunk");
                                            if facing_data.is_transparent() {
                                                ...
                                                let block = if let Some(block_data) =
                                                    registry.block_registry().by_id(block as usize)
                                                {
                                                    block_data.get_face(
                                                ...
                                                if b_data.is_transparent() {
                                                    transparent_mesh.add_face(
                                                        Point3::new(x as f32, y as f32, z as f32),
                                                        *direction,
                                                        block,
                                                    );
                                                } else {
                                                    opaque_mesh.add_face(
                                                        Point3::new(x as f32, y as f32, z as f32),
                                                        *direction,
                                                        block,
                                                    );
                                    ...
                                    _ => {
                                        let custom_mesh = registry
                                            .mesh_registry()
                                            .by_id(b_mesh)
                                            .expect("Unknown mesh in chunk");
                ...
                if opaque_mesh.vertices.len() != 0 || transparent_mesh.vertices.len() != 0 {
                    let transparent_mesh = if transparent_mesh.vertices.len() != 0 {
