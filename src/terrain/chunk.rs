...
impl FromWorld for ChunkPosition {
    fn from_world(x: f32, y: f32, z: f32) -> ChunkPosition {
        ChunkPosition::new(
            (x / (CHUNKSIZE) as f32).floor() as isize,
            (y / (CHUNKSIZE) as f32).floor() as isize,
            (z / (CHUNKSIZE) as f32).floor() as isize,
        )
    }
}
...
impl Chunk {
    ...
    pub fn get_block(&self, x: usize, y: usize, z: usize) -> usize {
        if x >= CHUNKSIZE {
            panic!("Accessing invalid block: {:?}", [x, y, z])
        }
        if y >= CHUNKSIZE {
            panic!("Accessing invalid block: {:?}", [x, y, z])
        }
    ...
    pub fn check_block(
        ...
        y: isize,
        z: isize,
        neighbors: Vec<Option<Arc<Chunk>>>,
    ) -> usize {
